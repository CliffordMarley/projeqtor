...
//     		      session_write_close();
        ...
        if ($class=='History' or $class=='Note') {
        ...
        foreach ($list as $obj) {
          if ($class=='History' or $class=='Note') {
            $objHistClass=$obj->refType;
            $objHist=new $objHistClass($obj->refId,true);
            if (securityGetAccessRightYesNo('menu'.$objHistClass, 'read', $objHist)!="YES") continue;
          }
        	if ($cpt) echo ",";
        	$cpt++;
