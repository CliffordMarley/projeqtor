...
foreach($tab as $res=>$lists) {
  ...
  foreach($arrDates as $date) {
    if ($scale=='week') {
      $firstDay=date('Y-m-d',firstDayofWeek(pq_substr($date,4,2),pq_substr($date,0,4)));
      $lastDay=addDaysToDate($firstDay, 7);
    } else {
      $firstDay=pq_substr($date,0,4) . '-' . pq_substr($date,4,2) . '-01';
      $lastDay=pq_substr($date,0,4) . '-' . pq_substr($date,4,2).'-'.lastDayOfMonth(intval(pq_substr($date,4,2)),pq_substr($date,0,4));
    }
    $capa=0;
