...
$showAdminProj=pq_trim(RequestHandler::getValue('showAdminProj'));
$paramTeam='';
...
if ($idResource!='') {
  ...
  if(!$res->isResourceTeam){
  	...
  	$idResList = SqlList::getListWithCrit('ResourceTeamAffectation', array('idResourceTeam'=>$idResource), 'idResource');
  	$queryWhere.=  " and t1.idResource in (".$idResource.",".implode(',', $idResList).")";
  }
}
...
$plannedStyle=' style="width:5px;margin:0;padding:0;text-align:center;background-color:' . $plannedBGColor . '; color: ' . $plannedFrontColor . ';" ';
 
$zoom=100;
$nb=count($arrDates);
if ($nb>15) {
  $zoom=intval(100-(($outMode=='pdf')?50:35)*($nb-15)/(52-15));
}
echo "<table width='95%' align='center' style='font-size:$zoom%;'><tr>";
echo '<td>';
...
echo '<td class="reportTableHeader" rowspan="2" style="width:10px;">' . i18n('sum') . '</td>';
echo '</tr>';
echo '<tr>';
$arrSum=array();
...
foreach($tab as $proj=>$lists) {
  ...
  for ($i=1; $i<=2; $i++) {
    if ($i==1) {
      echo '<tr><td class="reportTableLineHeader" style="width:'.intval(200*$zoom/100).'px;" rowspan="2">' . htmlEncode($lists['name']) . '</td>';
      $style=' ';
      $mode='real';
      $ital=false;
    } else {
...
foreach ($arrSum as $date=>$val) {
  echo '<td class="reportTableHeader" style="margin:0;padding:0 0 0 1px;border:1px solid white;">' . Work::displayWork($val) . '</td>';
  $sum+=$val;
  $cumul[$date]=$sum;
  $cumulUnit[$date]=Work::displayWork($sum);
}
echo '<td class="reportTableHeader" style="margin:0;padding:0 0 0 1px;">' . Work::displayWork($sum) . '</td>';
echo '</tr>';
echo '</table>';
echo '</td></tr></table>';
