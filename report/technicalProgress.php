...
//   }
  ...
  if (Sql::$lastQueryNbRows > 0) {
    ...
    while ($line = Sql::fetchLine($result)) {
      ...
      echo '    <TD class="reportTableData" style="border-left:0px; text-align: left;' . $compStyle . '" nowrap>' . $tab .(($outMode=='excel')?$line['refname']:htmlEncode($line['refname'])) . '</TD>';
      echo '    <TD class="reportTableData" style="' . $compStyle . '">' .round(pq_nvl($weight,0)). '</TD>' ;
      ...
      echo '    <TD class="reportTableData" style="width:69px;' . $compStyle . '">' . percentFormatter(round(pq_nvl($progress,0))) . '</TD>' ;
      echo '</TR>';
      }else{
        echo encodeCSV($line['refname']) .';';
        echo $weight.';';
        echo $startDate.';';
        echo $endDate.';';
        echo $assignedWork.';';
