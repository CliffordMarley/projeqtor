...
if ($paramYear >= '2000'  && ($paramStartMonth <= '12' && $paramStartMonth > '0')) {
  ...
  foreach($projects as $idP=>$nameP) {
    $countResourcesByProject[$idP] =0;
    foreach($activities as $idA=>$nameA) {
      if (array_key_exists($idA, $result[$idP])){
        $countResourcesByProject[$idP]+=count($result[$idP][$idA]);
      }
    }
  }
  ...
  foreach ($projects as $idP=>$nameP) {
    ...
    echo '<tr height="20px"><td class="reportTableLineHeader" style="width:150px;" rowspan="'. ($countResourcesByProject[$idP]+1) . '" '.excelFormatCell('rowheader',20).'>' . htmlEncode($nameP) . '</td>';
    foreach($activities as $idA=>$nameA) {
      if (array_key_exists($idA, $result[$idP])){
