...
// MTY - LEAVE SYSTEM
        if ( (getSessionValue('project')!='*' and !$showAllProjects) or getSessionValue('idFavoriteProjectList') ) {
          $queryWhere.= ($queryWhere=='')?'':' and ';
...
// MTY - LEAVE SYSTEM
    ...
    if ($print) {
        ...
        foreach($arrayFieldsWithCase as $key => $val) {
          ...
          if(isset($arrayFieldsWithCase[$key]) and pq_substr($arrayFieldsWithCase[$key], 0, 1) == "["){
    		...
    		while ($line = Sql::fetchLine($result)) {
    		  if ($first) {
	    			foreach ($line as $id => $val) {
	    			  if (!isset($arrayFields[pq_strtolower($id)]) || ($objectClass=='GlobalView' and $id=='id')) {
	    			    continue;   
	    			  }
	    				$colId=$id;
	          ...
	          if(array_key_exists($id, $arrayUORefName)){
	            ...
	            foreach ($arrayUORefName as $nameUO){
            	  ...
            	  $titleUO .= encodeCSV(pq_str_replace('_', ' - ', $nameUO)).$csvSep;
	            }
	            echo $titleUO.$csvSep;
	          }
	          echo "\r\n";
            ...
            if ( ( isset($dataType[$id]) and $dataType[$id]=='varchar' or $foreign) and $csvQuotedText) { 
              ...
              if ($objectClass == 'Work' and Parameter::getGlobalParameter('imputationUnit')=='hours' and $id=='work') $val=GeneralWork::displayImputation($val);
        ...
        echo '<br/>';
	      ...
	      if ($objectClass=='GlobalView' ) $layout=pq_str_replace('<th class="reportHeader" field="id" style="border:1px solid black;width:0%">id</th>','',$layout);
	      echo $layout;
	      echo '</tr>';
	      if (Sql::$lastQueryNbRows > 0) {
	        ...
	        while ($line = Sql::fetchLine($result)) {
	          ...
	          foreach ($line as $id => $val) {
	            ...
	            echo '<td class="tdListPrint '.((pq_substr($formatter[$numField],0,5)=='color')?'colorNameData':'').'" style="white-space:normal;width:' . $colWidth . ';">' . $disp . '</td>';
	          }
	          echo '</tr>';       
	        }
	      }
