...
$columns=Parameter::getPlanningColumnOrder($planningType);
...
foreach ($columnsAll as $order=>$col) {
    if ( (isset($resourcePlanning) and ($col=='ValidatedWork' or $col=='Resource' or pq_substr($col,-4)=='Cost' or $col=='Responsible' or $col=='RespInitial') )
        or (isset($portfolioPlanning) and ($col=='Resource' or $col=='IdPlanningMode' or $col=='Responsible' or $col=='RespInitial') )
        or (isset($versionPlanning) and (pq_substr($col,-4)=='Cost' or $col=='Responsible' or $col=='RespInitial'))
        or (isset($contractGantt) and ($col=='Responsible' or $col=='RespInitial'))) {
        // nothing
	  ...
	  if (isNewGui()) {
  	  ...
  	  echo '<span dojoType="dijit.form.CheckBox" type="checkbox" id="checkColumnSelector'.$col.'" style="display:none" ' 
  	    ...
  	    . (($col=='Name')?' readonly':'')
  	    . ' onChange="changePlanningColumn(\'' . $col . '\',this.checked,\'' . $order . '\',\'' . $planningType . '\');" '
  	  ...
  	  echo ' onChange="changePlanningColumnWidth(\'' . $col . '\',this.value,\'' . $planningType . '\')"; ';
