...
$version = "V10.3.2"; // Version of application : Major / Minor / Release
$build = "0331"; // Build number. To be increased on each release
...
if (!(isset($maintenance) and $maintenance) and !(isset($batchMode) and $batchMode) and !(isset($indexPhp) and $indexPhp)) {
  ...
  $pos=strrpos($page, "/");
  if(pq_trim(getSessionValue('uniqueSessionId')) and (strrpos($page, "/tool") or strrpos($page, "/view")) and !(pq_substr($page, strrpos($page, "/view/"))=='/view/main.php')){
...
// MTY - LEAVE SYSTEM
    case 'EmploymentContract' :
      ...
      break;
// MTY - LEAVE SYSTEM
    case 'Resource': case 'ResourceAll': case 'ResourceMain' :
...
function getObjectClassFieldsListWithDateType($objectClassName='', $withoutCreationDate=true) {
  ...
  foreach ($obj as $colName=>$val) {
    if ($colName!='date' and (pq_strpos($colName, 'Date')===false or pq_substr($colName, 0, 1)=='_')) continue;
...
function setSessionValue($code, $value, $global=false) {
  global $paramDbName, $paramDbPrefix,$simuIndex;
  if ($global) {
    $projeqtorSession='ProjeQtOr'.$simuIndex;
  } else {
    $projeqtorSession='ProjeQtOr_'.$simuIndex.$paramDbName.(($paramDbPrefix)?'_'.$paramDbPrefix:'');
  }
...
function getListForSpecificRights($specific, $includePool=false, $includeUsers=false, $includeMaterial=true) {
  ...
  if ($user->allSpecificRightsForProfilesOneOnlyValue($specific, 'NO')) {
    ...
    $table=SqlList::getList($class,'name',null,false,true);
