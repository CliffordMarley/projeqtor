...
$columnsDescription=Parameter::getPlanningColumnDescription('resource');
$obj=new $objectClass();
...
if (array_key_exists('selectResourceName',$_REQUEST)) {
  $selectResource=pq_trim($_REQUEST['selectResourceName']);
}
...
if (array_key_exists('teamName',$_REQUEST)) {
  $selectTeam=pq_trim($_REQUEST['teamName']);
}
...
if (array_key_exists('organizationName',$_REQUEST)) {
  $selectOrganization=pq_trim($_REQUEST['organizationName']);
}
...
if ( array_key_exists('report',$_REQUEST) ) {
	...
	if (array_key_exists('format',$_REQUEST)) {
		Security::checkValidDatePeriodScale(pq_trim($_REQUEST['format']));
...
and ( ! array_key_exists('idProject',$_REQUEST) or pq_trim($_REQUEST['idProject'])=="")) {
    ...
    echo ' "items":[';
		...
		foreach ($list as $line) {
			...
			foreach ($line as $id => $val) {
				...
				if ($id=='refname' or $id=='resource' or $id=='planningmode' or pq_substr($id,-7)!='display') { 
        ...
        echo '"' . htmlEncode($id) . '":"' . $val . '"';
			...
			echo ', "color":"'.((isset($line['color']))?pq_trim($line['color'],'#'):'').'"';
			echo '}';
		}
		echo ' ] }';
	}
  ...
  if ( array_key_exists('showWork',$_REQUEST) ) {
	...
	if (count($list) > 0) {
      ...
      $maxDate=pq_substr($maxDate,0,5).$arrayMax[pq_substr($maxDate,5)];
		...
		// Header
	  $sortArray=Parameter::getPlanningColumnOrder('resource');
    $cptSort=0;
    foreach ($sortArray as $name) { if (pq_substr($name,0,6)!='Hidden' and $name!='ValidatedWork' and $name!='Resource') $cptSort++; }
