...
function drawOptionsDisplay() {
  global $saveShowWbs, $saveShowClosed, $saveShowResource,$planningType, $showListFilter,$showClosedPlanningVersion,$showColorActivity;?>
  <table width="100%">
...
function drawOptionHideAssignationWihtoutLeftWork() {
  ?>
  <div style="white-space:nowrap; <?php echo (isNewGui())?'margin-right:6px;margin-top:5px;':'position:absolute; bottom:5px;left:10px;'; ?>" class="checkboxLabel">
    <?php if (isNewGui()) {?><?php echo pq_ucfirst(i18n('hideAssignationWihtoutLeftWork'));?>&nbsp;<?php }?>
    <span title="<?php echo pq_ucfirst(i18n('hideAssignationWihtoutLeftWork'));?>" dojoType="dijit.form.CheckBox"
      type="checkbox" id="hideAssignationWihtoutLeftWork" name="hideAssignationWihtoutLeftWork" class="whiteCheck"
      <?php if ( Parameter::getUserParameter('hideAssignationWihtoutLeftWork')=='1') {echo 'checked="checked"'; } ?>  >  
      <script type="dojo/connect" event="onChange" args="evt">
        saveUserParameter('hideAssignationWihtoutLeftWork',((this.checked)?'1':'0'));
        refreshJsonPlanning();
      </script>                    
    </span>
    <?php if (!isNewGui()) {?>&nbsp;<?php echo i18n('hideAssignationWihtoutLeftWork');?><?php }?>
  </div>
<?php 
}
