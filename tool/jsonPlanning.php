  ...
  if ($print) {
    ...
    if ($nbQueriedRows > 0) {
      ...
      while ($line = Sql::fetchLine($result)) {
        ...
        if (isset($collapsedList['Planning_'.$refItemCollapsed])) {
        	echo ',"collapsed":"1"';
        } else {
        	echo ',"collapsed":"0"';
        }
	        ...
	        foreach ($assList as $ass) {       	
	        	...
	        	if (! isset($arrayResource[$res->id])) {
  	        	...
  	        	if ($displayResource=='initials' and ! $display) {
  	        	  ...
  	        	  //$display=$encoding;
  	        	  if ($res->name) 
  	        	    $words=mb_split(' ',pq_str_replace(array('"',"'"), ' ', $res->name));
  	        	  else 
  	        	    $words=array();
  	        	  $display='';
        ...
        if (!$portfolio and $line['reftype']!='Fixed' and $line['reftype']!='Construction' and $line['reftype']!='Replan') {
          ...
          if($resp){
            ...
            if(!$display or (is_numeric($display) and Security::checkValidId($display))){
              $display='';
              $words=mb_split(' ',pq_str_replace(array('"',"'"), ' ', $responsible));
              foreach ($words as $word) {
                $display.=(pq_mb_substr($word,0,1,'UTF-8'));
              }
            }
            if($responsible)echo ',"responsible":"' . $responsible. '"';
            if($display)echo ',"respinitial":"' . $display. '"';
          }else{
            echo ',"responsible":""';
            echo ',"respinitial":""';
          }
        ...
        echo ', "color":"'.pq_trim(((isset($line['color']))?$line['color']:''),'#').'"';
        echo '}';
        $cp++;
        if($cp==$limitDisplay){
