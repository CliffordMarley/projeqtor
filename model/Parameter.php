...
// MTY - GENERIC DAY OFF
  ...
  static public function getPlanningColumnDescription($planningType='planning') {
    ...
    foreach($arrayFields as $col=>$width) {
      $cpt++;
      self::$planningColumnDescription[$planningType][$col]=array('name'=>$col,'show'=>1,'order'=>$cpt,'width'=>$width);
    }
    ...
    foreach($arrayFieldsSorted as $order=>$column) {
      if(self::$planningColumnDescription[$planningType][$column]['show']==1){
        $res[$i]=$column;
      }else{
        $res[$i]='Hidden'.$column;
      }
