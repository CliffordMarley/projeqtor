...
class TestSessionMain extends SqlElement {
  ...
  private static $_doNotUpdateDependencies=false;
...
/** =========================================================================
  ...
  public function control(){
    $result="";
    $old=$this->getOld();
    if (!pq_trim($this->idProject) and !pq_trim($this->idProduct)) {
    ...
    $parentTestSession = new TestSession($this->idTestSession);
    
    if ($parentTestSession->idTestSession == $this->id and $this->id!=null) {
      $result.="<br/>" . i18n('errorHierarchicLoop');
    }
    
    if ($parentTestSession->idProject != null and $this->idProject != $parentTestSession->idProject ) {
      $result.="<br/>" . i18n('cantAssociateATestSessionWithoutSameProject') ;
    }
    
    if ($this->TestSessionPlanningElement and $this->TestSessionPlanningElement->id
      and ($this->idActivity!=$old->idActivity or $this->idProject!=$old->idProject) and $result==''){
  ...
  public function save() {
  	$old=$this->getOld();
  	if (! pq_trim($this->idRunStatus)) $this->idRunStatus=5;
    ...
    self::$_doNotUpdateDependencies=true; // To avoid recursive loop on update dependency when idle=1 that is dispatched to all TestCaseRun
  	$result=parent::save();
  	self::$_doNotUpdateDependencies=false;
  	if(getLastOperationStatus($result) == 'OK')	$this->updateDependencies();
    return $result;
  }
  ...
  public function copyTo($newClass, $newType, $newName, $newProject, $setOrigin, $withNotes, $withAttachments, $withLinks, $withAssignments = false, $withAffectations = false, $toProject = null, $toActivity = null, $copyToWithResult = false, $copyToWithActivityPrice=false, $copyToWithStatus = false, $copyToWithSubTask=false) {
    $newObj=parent::copyTo($newClass, $newType, $newName, $newProject, $setOrigin, $withNotes, $withAttachments, $withLinks, $withAssignments, $withAffectations, $toProject, $toActivity, $copyToWithResult, $copyToWithActivityPrice, $copyToWithStatus);
    $new=$this->copyTestCasRun($newObj);
    return $new;
  }
  ...
  public function updateDependencies() {
    if (self::$_doNotUpdateDependencies) return;
   ...
   public function getCalculatedItem(){
     ...
     if ($this->id) {
       ...
       $this->runStatusIcon='<img src="../view/css/images/icon'.pq_ucfirst($name).'22.png" />';
     }
  }
