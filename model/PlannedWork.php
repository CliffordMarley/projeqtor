...
// ================================================================================================================================
  ...
  public static function plan($projectIdArray, $startDate,$withCriticalPath=true,$infinitecapacity=false,$simulation=false,$criticalLike=false) {
    ...
    global $cronnedScript, $listProjectsType;
    $forceFDUR=false;
    $forceASAP=false;
...
//       }
    ...
    $fullListPlan=PlanningElement::initializeFullList($list,$forceASAP);
    ...
    $listPlan=self::sortPlanningElements($fullListPlan, $listProjectsPriority,$listProjectsType);
    $resources=array();
    $a=new Assignment();
    $topList=array();
    $reserved=array();
...
//-- Treat each PlanningElement ---------------------------------------------------------------------------------------------------
    ...
    foreach ($listPlan as $plan) {
      ...
      $typeProject=(isset($listProjectsType[$plan->idProject]))?$listProjectsType[$plan->idProject]:'';
...
//       }
        ...
        if ($plan->refType=='Milestone') {
          ...
          if (count($plan->_predecessorListWithParent)>0) {
            ...
            foreach ($plan->_predecessorListWithParent as $precId=>$precValArray) { // $precValArray = array(dependency delay,dependency type)
              ...
              $calcDate=addWorkDaysToDate($precEnd, $precVal);
      ...
      if ($plan->refType=='Milestone') {
        ...
        if ($profile=="FDUR") {
          if (! $plan->realStartDate) {
            if ($plan->elementary) {
              if ($plan->plannedWork==0) $plan->plannedStartDate=$startPlan;
        ...
        if ($withProjectRepartition) {
          ...
          if ($infinitecapacity) $listTopProjects=array($plan->idProject);
        ...
        if ($profile=="ASAP" and $forceASAP) {
          $lastPlanDate=$plan->getExpectedEndDate($listPlan);
        }
...
//          $idxAss=0;
          ...
          foreach ($listAss as $keyAss=>$ass) {
            ...
            if ($startPlan>$startDate) {
              ...
              if ($step==-1) {
                if ($realProfile=='START' or $profile=="STARR") $currentDate=$plan->validatedStartDate;
                $step=1;
              }
            }
            if ($profile=='GROUP') {
            ...
            if ($profile=="REGUL" or $profile=="FULL" or $profile=="HALF" or $profile=="QUART" or $profile=="FDUR") {
              ...
              if ($profile=="REGUL" or $profile=="FULL" or $profile=="HALF" or $profile=="QUART") {
            	...
            	for($tmpDate=$currentDate; $tmpDate<=$endPlan;$tmpDate=addDaysToDate($tmpDate, 1)) {
            		...
            		if ($tempCapacity>=$regulTh or $regulTh==0 or $forceFDUR or $plan->elementary==0) {
            			$delai+=1;
            		} else {
            ...
            while (1) {
              if (($realProfile=="START" or $profile=="STARR") and $currentDate<$plan->validatedStartDate and $step==-1) {
                $currentDate=$plan->validatedStartDate;
                $step=1;
              }
              ...
              $surbooked=0;
              $surbookedWork=0;
              if ($withProjectRepartition and isset($reserved['W']) and ! ($forceFDUR and ($profile=='FDUR' or $profile=='REGUL' or $profile=='FULL' or $profile=='HALF' or $profile=='QUART'))) {
              ...
              if ($ress['variableCapacity'] or $infinitecapacity) {
                ...
                if ($forceFDUR and ($profile=='FDUR' or $profile=='REGUL' or $profile=='FULL' or $profile=='HALF' or $profile=='QUART')) {
                  $capacityRate=$regul;
                } else  if ($ress['team']) {
                  $capacityRate=$ass->capacity;
                } else {              
                    ...
                    if (isset($ress['periods'][$period]['idResource'][$idMember])) {
                      ...
                      if (isset($member[$currentDate])) {
                        ...
                        if (isset($reserved['W']['sum'][$ass->idResource])) {
                          ...
                          if (isset($reserved['W']['sum'][$ass->idResource][$dow])) $tmpCapa-=$reserved['W']['sum'][$ass->idResource][$dow];
                        }                      
                      }
...
//               } else 
              ...
              if (isOpenDay($currentDate, $r->idCalendarDefinition)) {            
                ...
                if ( ! ($planned < $capacity or $profile=='RECW') )  {
                  ...
                  if ($withProjectRepartition and $profile!='RECW' and ! ($forceFDUR and ($profile=='FDUR' or $profile=='REGUL' or $profile=='FULL' or $profile=='HALF' or $profile=='QUART'))) {
                    foreach ($listTopProjects as $idProject) {
...
//                         }
                  ...
                  if ($value>=0.01) {
                    self::storePlannedWork(
                        ...
                        $left, $ress, null, $infinitecapacity);
                        $surbookedWork=0;
                    // Support Resource
                    if (count($ress['support'])>0) {
                      foreach ($ress['support'] as $sup=>$supRate) {
                        ...
                        if ($valueSup>0) {
                          ...
                          self::storePlannedWork(
                            ...
                            $leftSup, $supRes, $sup, $infinitecapacity);
...
// Functions for PLAN
  ...
  private static function storeReservedForRecurring() {
    global $listPlan,$reserved,$listProjectsType;
    $memberOf=array();
              ...
              if ($pred->refType=='Milestone' and $pred->validatedEndDate and ($pred->_profile=='FIXED' or ($pred->_profile=='FLOAT' and count($pred->_directPredecessorList)==0))) {
                if ($dep->dependencyType=='E-S' or $dep->dependencyType=='S-S') {
                  if ($dep->dependencyDelay>0) {
                    $reserved[$artype][$plan->id]['start']=addWorkDaysToDate($pred->validatedEndDate,$dep->dependencyDelay+1);
                  } else {
                    $reserved[$artype][$plan->id]['start']=addWorkDaysToDate($pred->validatedEndDate,$dep->dependencyDelay);
                  }
                } else if ($dep->dependencyType=='E-E') {
                  if ($dep->dependencyDelay>0) {
                      $reserved[$artype][$plan->id]['end']=addWorkDaysToDate($pred->validatedEndDate,$dep->dependencyDelay+1);
                    } else {
                      $reserved[$artype][$plan->id]['end']=addWorkDaysToDate($pred->validatedEndDate,$dep->dependencyDelay);
                    }
                  }
              }
            }
  ...
  private static function sortPlanningElements($list,$listProjectsPriority, $listProjectsType) {
    ...
    foreach ($list as $id=>$elt) {
      ...
      if (isset($listProjectsType[$elt->idProject])) {
        ...
        if ($typeProject=='TMP' ) $projPrio='6';
        else if ($typeProject=='PRP' ) $projPrio='5';
      } else {
        $projPrio='9';
      }
      $pm=$pmList[$elt->idPlanningMode];
    	if ($elt->elementary==0) {
    	  ...
    	  $crit='3'.pq_str_replace('-','',$elt->validatedStartDate);
    	  ...
    	  $crit='6'.$str01; // Lower priority (availability will be reserved)
    		...
    		$crit='5'.'.'.pq_str_replace('-','',$elt->validatedEndDate);
      ...
      if ($pm=='ALAP' and $elt->validatedEndDate and $mainPriority=='priority') {
        $crit.='.'.pq_str_replace('-','',$elt->validatedEndDate);
      } else {
        $crit.=$str99;
      }
      ...
      if ($elt->elementary==0) { $crit.='.99999';}
  ...
  private static function storePlannedWork(
      ...
      &$surbooked, &$surbookedWork, &$ass, &$plan, &$arrayPlannedWork, &$changedAss,
      &$left, &$ress, $support=null, $infinitecapacity=false) {
    if (!$support) {
      if (!isset(self::$_surbookedWorkStored[$ass->idResource])) self::$_surbookedWorkStored[$ass->idResource]=array();
      ...
      if ( $value+$planned > $r->getCapacityPeriod($currentDate)) {       
        ...
        if ($value>$capacityNormal) {
          $surbookedWork=$value-self::$_surbookedWorkStored[$ass->idResource][$currentDate]-$capacityNormal;
        }
      }
      if ($surbookedWork>0 and $plannedReserved>0 and $infinitecapacity) {
  ...
  private static function storePlannedWorkLeveled($dateAfter, &$ass, &$plan, &$arrayPlannedWork, &$changedAss, &$left) {
    if ($left==0) return;
    $end=($ass->plannedEndDate)?$ass->plannedEndDate:addWorkDaysToDate($dateAfter, -1);
    $start=($ass->plannedStartDate)?$ass->plannedStartDate:$end;
    $duration=workDayDiffDates($start, $end);
    ...
    for ($currentDate=$start; $currentDate<=$end;$currentDate=addDaysToDate($currentDate,1)) {
      ...
      $arrayPlannedWork[$idPW]=$plannedWork;
    }
