		...
		foreach($data as $nbl=>$fields){
          ...
          continue;
					...
					if (isset($titleObject[$idx])) {
					  ...
					  if (property_exists($subClass, $fldName)) {
					    if (pq_substr($fldName, 0, 2) == 'id' and pq_substr($fldName, 0, 4) != 'idle' and pq_strlen($fldName) > 2 and !is_numeric($field) and $field!=null) {
					      $obj->$subClass->$fldName = SqlList::getIdFromName(pq_substr($fldName, 2), $field);
					    } else {
						...
						if (pq_substr($fldName, 0, 2) == 'id' and pq_substr($fldName, 0, 4) != 'idle' and pq_strlen($fldName) > 2 and !is_numeric($field) and $field!=null) {
						    ...
						    $obj->$fldName = SqlList::getIdFromName(pq_substr($fldName, 2), $field, (($fldName=='id'.get_class($obj))?true:false));
							}
						} else {
						    if(($fldName == 'work' or $fldName == 'leftWork') and $convertWork){
				...
				if (pq_stripos($result, 'id="lastOperationStatus" value="ERROR"') > 0) {
					...
					self::$cptWarning+=1;
					if (pq_stripos($result, 'id="lastOperationStatus" value="INVALID"') > 0) {
						self::$cptInvalid+=1;
					} else if (pq_stripos($result, 'id="lastOperationStatus" value="NO_CHANGE"') > 0) {
						self::$cptUnchanged+=1;
					} else {
						// ???
					}
