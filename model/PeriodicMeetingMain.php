...
// ============================================================================**********
  ...
  public function drawSpecificItem($item){
    ...
    $result="";
    if ($item=='periodicity') {
    	//$result.='<div style="display:'.(($this->idPeriodicity==1)?'block':'block').'" id="DAY">';
    	$result.='<div style="display:block" id="DAY">';
    ...
    if($item=='buttonProjectDate'){
      $result .= '<tr><td valign="top" class="label"><label></label></td><td>';
      $result .= '<button style="" class="dynamicTextButton" id="buttonProjectDate" dojoType="dijit.form.Button" showlabel="true" onClick ="setProjectDateToMeeting();"';
      $result .= ' title="' . i18n('buttonProjectDateTitle') . '" >';
      $result .= '<span>' . i18n('buttonProjectDate') . '</span>';
      $result .= '</button>';
      $result .= '</td></tr>';
    }
  ...
  public function delete() {
  	...
  	foreach ($lstMeet as $meet) {
  		...
  		if (pq_stripos($resDel,'id="lastOperationStatus" value="OK"')==0 ) {
        return $resDel;
      }    
  	}	
  	...
  	if (pq_stripos($result,'id="lastOperationStatus" value="OK"')>=0 ) {
	    ...
	    foreach ($lstMeet as $meet) {
	      ...
	      $resMaj=$meeting->save();
	      if (pq_stripos($resMaj,'id="lastOperationStatus" value="OK"')==0 ) {
	        return $resMaj;
	      }    
	    } 
  	}
  ...
  public function save() {
    ...
    if($this->idle!=1 and $old->idle==0){
      $nb=0;
      $currentDate=$this->periodicityStartDate;
      $maxDate=addMonthsToDate($currentDate, 36);
      $nbWeekDay=0;
      if ($this->idPeriodicity==4) {
      	$currentDate=pq_substr($currentDate,0,8).'01';
      } else if ($this->idPeriodicity==5) {
        $currentDate=pq_substr($currentDate,0,4).'-'.htmlFixLengthNumeric($this->periodicityYearlyMonth,2)
                                             .'-'.htmlFixLengthNumeric($this->periodicityYearlyDay,2);
      }    
      $currentMonth=pq_substr($currentDate,5,2);
      ...
      while ( (    ($this->periodicityEndDate and $currentDate<=$this->periodicityEndDate) 
        ...
        if ($this->idPeriodicity==2) { // WEEKLY
          if ($this->periodicityWeeklyDay==date('N', pq_strtotime($currentDate)) ) {
            if (! $this->periodicityOpenDays or isOpenDay($currentDate,'1')) {       	
  	          $nb++;
  	          $this->saveMeeting($currentDate, $nb, $old);
  	          $lastDate=$currentDate;
        ...
        if ($this->idPeriodicity==3) { // MONTHLY DAY
          if ($this->periodicityMonthlyDayDay==pq_substr($currentDate,8,2)) {
            if (! $this->periodicityOpenDays or isOpenDay($currentDate,'1')) {        
              $nb++;
              $this->saveMeeting($currentDate, $nb, $old);
              $lastDate=$currentDate;
            ...
            $currentDate=pq_substr($currentDate,0,8).htmlFixLengthNumeric($this->periodicityMonthlyDayDay,2);
            if (! $this->periodicityOpenDays or isOpenDay($currentDate,'1')) {        
              $nb++;
              $this->saveMeeting($currentDate, $nb, $old);
              $lastDate=$currentDate;
        ...
        if ($this->idPeriodicity==4) { // MONTHLY WEEK
        	if ($this->periodicityMonthlyWeekDay==date('N', pq_strtotime($currentDate)) ) {
        		$nbWeekDay+=1;
        		if ($nbWeekDay==$this->periodicityMonthlyWeekNumber) {
        			if ( (! $this->periodicityOpenDays or isOpenDay($currentDate,'1') )
        			  and $currentDate>=$this->periodicityStartDate 
        			  and pq_substr($currentDate,5,2)==$currentMonth ) {  
        				$nb++;
                $this->saveMeeting($currentDate, $nb, $old);
                $lastDate=$currentDate;
        			}
        			$nbWeekDay=0;
        			$currentDate=pq_substr($currentDate,0,8).'01';
        			if ($currentMonth==pq_substr($currentDate,5,2)) {
  ...
  private function saveMeeting($currentDate, $nb, $old) {
  	...
  	if (!$isNew and ($meeting->done==1 or $meeting->idle==1 or $meeting->MeetingPlanningElement->realWork>0)) return; // Do not change a meeting already done
    ...
    if ($isNew) {
      ...
      foreach ($assList as $assPeriodic) {
        if($currentDate < date('Y-m-d'))continue;
        $res = new ResourceAll($assPeriodic->idResource, true);
        if($res->startDate != '' and $res->startDate > $currentDate)continue;
        if($res->endDate != ''  and $res->endDate < $currentDate)continue;
        $ass=new Assignment();
        $ass->refType='Meeting';
  ...
  public function copyTo($newClass, $newType, $newName, $newProject, $setOrigin, $withNotes, $withAttachments,$withLinks, $withAssignments=false, $withAffectations=false, $toProject=null, $toActivity=null, $copyToWithResult=false, $copyToWithActivityPrice=false, $copyToWithStatus=false, $copyToWithSubTask=false){
    $result = parent::copyTo($newClass, $newType, $newName, $newProject, $setOrigin, $withNotes, false, false,false,false,$toProject,$toActivity, $copyToWithResult, $copyToWithActivityPrice, $copyToWithStatus);
    $ass=new Assignment();
