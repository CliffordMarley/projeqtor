...
//scriptLog('Cron::checkImport()');
  ...
  public static function checkEmails() {	
    ...
    foreach ($lstIMb as $mb){ 
      ...
      disableCatchErrors();
      if(!$mailsIds) {
        debugTraceLog("Mailbox #$mb->id $mb->serverImap for $mb->imapUserAccount is empty (filter='$imapFilterCriteria')"); // Will be a debug level trace
        if ($mb->failedRead>0) {
          $mb->failedRead=0;
          $resMb=$mb->save();
        }
      ...
      foreach ($mailsIds as $mailId){
        ...
        // Case 1 => answer to mail senty from projeqtor : add message as note to item
          ...
          if($result == ""){
          	...
          	if(getLastOperationStatus($resultTicket)=='OK' and $mb->allowAttach==1){
          		...
          		foreach ($listAtt as $att){
          			...
          			if (pq_substr($ext,0,3)=='php' or pq_substr($ext,0,3)=='pht' or pq_substr($ext,0,3)=='sht') {
