...
// END - ADD BY TABARY - NOTIFICATION SYSTEM      
  ...
  public function copyTo($newClass, $newType, $newName, $newProject, $setOrigin, $withNotes, $withAttachments, $withLinks, $withAssignments = false, $withAffectations = false, $toProject = null, $toActivity = null, $copyToWithResult = false, $copyToWithActivityPrice=false, $copyToWithStatus = false,$copyToWithSubTask = false) {
    global $debugTraceUpdates,$debugTraceHistory;
    scriptLog("for ".debugDisplayObj($this)."->copyTo(newClass=$newClass, newType=$newType, newName=$newName, newProject=$newProject, setOrigin=$setOrigin, withNotes=$withNotes, withAttachments=$withAttachments, withLinks=$withLinks, withAssignments=$withAssignments, withAffectations=$withAffectations, toProject=$toProject, toActivity=$toActivity, copyToWithResult=$copyToWithResult, copyToWithStatus=$copyToWithStatus, copyToWithSubTask=$copyToWithSubTask)");
    if (isset($debugTraceUpdates) and $debugTraceUpdates==true) {
      if ( ! property_exists($this,'_noHistory') or (isset($debugTraceHistory) and $debugTraceHistory==true) )
        debugTraceLog("Start COPYTO for ".get_class($this)." #".$this->id);$startMicroTime=microtime(true);
    }
    self::setCopyInProgress();
    $result=$this->copySqlElementTo ( $newClass, $newType, $newName, $newProject, $setOrigin, $withNotes, $withAttachments, $withLinks, $withAssignments, $withAffectations, $toProject, $toActivity, $copyToWithResult, $copyToWithStatus,$copyToWithSubTask);
    if (isset($debugTraceUpdates) and $debugTraceUpdates==true) {
      if ( ! property_exists($this,'_noHistory') or (isset($debugTraceHistory) and $debugTraceHistory==true) )
  ...
  private function saveSqlElement($force = false, $withoutDependencies = false, $forceInsert = false) {
    ...
    self::$_cachedQuery[get_class($this).'Type']=array();
  ...
  private function copySqlElementTo($newClass, $newType, $newName, $newProject, $setOrigin, $withNotes, $withAttachments, $withLinks, $withAssignments = false, $withAffectations = false, $toProject = null, $toActivity = null, $copyToWithResult = false, $copyToWithStatus = true,$copyToWithSubTask = false) {
    $newObj = new $newClass ();
    $newObj->id = null;
    ...
    if (Sql::$lastQueryNbRows > 0) {
      ...
      while ( $line ) {
        ...
        foreach ( $obj as $col_name => $col_value ) {
          ...
          } else if (pq_strpos ( $this->getFieldAttributes ( $col_name ), 'calculated' ) !== false) {
...
// END - ADD BY TABARY - TOOLTIP  
  ...
  public function setAllDefaultValues($onSave = false) {
    ...
    foreach ( $defaultValues as $field => $value ) {
      if ($onSave) {
        if ((! $this->$field and (!$this->id or $onSave==2)) or $this->isAttributeSetToField ( $field, 'readonly' )) {
          $def = $this->getDefaultValue ( $field, (! $this->id or $onSave==2)?true:false);
          if ($def !== null)
            $this->$field = $def;
        }
      } else {
  ...
  function getColCaption($fld) {
    ...
    return i18n ( 'col' . pq_ucfirst ( $fldName ) );
  }
...
// MTY - LEAVE SYSTEM        
  ...
  public function control() {
    ...
    if ($right != 'YES' and get_class ( $this ) == 'Project') {
      ...
      if ($right == "YES" and $this->idProject and $this->id) {
    ...
    foreach ( $this as $col => $val ) {
      ...
      if (pq_substr ( $col, 0, 1 ) != '_') {
        if (pq_ucfirst ( $col ) == $col and is_object ( $val )) {
          ...
          if ((pq_strpos ( $this->getFieldAttributes ( $col ), 'required' ) !== false or array_key_exists ( $col, $arrayExtraRequired )) 
            ...
            if ( ( ! $val) or pq_trim ( $val ) == '') { // Since PHP 7.1, all numeric fields have default value zero and cannot be null  
          ...
          if ($val and pq_strpos($this->getFieldAttributes($col),'unique')!==false) {
            ...
            if ($count>0) {
              $result .= '<br/>' . i18n ( 'messageUnique', array(i18n('col'.pq_ucfirst($col)),$val) );
            }
          }
          if ($dataType == 'datetime') {
            if (pq_strlen ( $val ) == 9) {
              $result .= '<br/>' . i18n ( 'messageDateMandatoryWithTime', array(i18n ( 'col' . pq_ucfirst ( $col ) )) );
            }
          }
          if ($dataType == 'date' and $val != '') {
            if (pq_strlen ( $val ) != 10 or pq_substr ( $val, 4, 1 ) != '-' or pq_substr ( $val, 7, 1 ) != '-') {
              $result .= '<br/>' . i18n ( 'messageInvalidDateNamed', array(i18n ( 'col' . pq_ucfirst ( $col ) )) );
            }
          }
        }
      }
      if ($val and $col != 'colRefName') {
        if ($dataType == 'varchar') {
          if (pq_strlen ( $val ) > $dataLength) {
            $result .= '<br/>' . i18n ( 'messageTextTooLong', array(i18n ( 'col' . pq_ucfirst ( $col ) ), $dataLength) );
          }
        } else if ($dataType == "int" or $dataType == "decimal") {
          if (pq_trim ( $val ) and ! is_numeric ( $val )) {
            $result .= '<br/>' . i18n ( 'messageInvalidNumeric', array(i18n ( 'col' . pq_ucfirst ( $col ) )."='".$val."'") );
          }
        }
      }
      if ($dataLength > 4000) {
        ...
        try {
          ...
          $result .= '<br/>' . i18n ( 'messageInvalidHTML', array(i18n ( 'col' . pq_ucfirst ( $col ) )) );
        }
        if (isTextFieldHtmlFormatted ( $val ))
    ...
    if (property_exists($this, $idType)) {
      ...
      if (property_exists ( $objType, 'mandatoryDescription' ) and $objType->mandatoryDescription and property_exists ( $this, 'description' )) {
        if (! $this->description) {
          ...
          if(intval(Parameter::getGlobalParameter('logLevel')>=3)) $result.='<br/><span style="font-size:70%">'.i18n('col'.ucfirst($idType))." '".SqlList::getNameFromId($type, $this->$idType)."'".'</span>';
        }
      }
...
//       }
  ...
  public function getMailDetail($directMessage=null) {
    ...
    $countInputMailBox = $inputmailbox->countSqlElementsFromCriteria(array('idle'=>'0'));
    ...
    if (isset ( $this->_DocumentVersion ) and is_array ( $this->_DocumentVersion )) {
      ...
      foreach ( $documentVersions as $documentVersion ) {
        ...
        if (pq_mb_strlen ( $plainText ) > 10000) { // Should not send too long email
  	...
  	if (pq_substr ( $col, 0, 4 ) == '_tab') {
  		...
  		if (is_array ( $val )) {
  			...
  			if (pq_mb_strlen ( $val ) > 1000000) {
  				...
  				$plainText = $text->getText();
  				$msg .= htmlSetClickableImages(nl2br(pq_mb_substr($plainText,0, 1000000)), 450);
  			} else {
...
public function getLastChangeTabForObject($obj,$lastChangeDate) {
  ...
  if ($obj->id) {
    ...
    if (property_exists($obj, 'PlanningElement')) {
...
public function getMailDetailFromTemplate($templateToReplace, $lastChangeDate=null, $isTitle=false) {
    ...
    if (((property_exists ( $type, 'lockPaused' ) and $type->lockPaused) or $force)) {
    	if (property_exists ( $this, 'paused' )) {
    		if ($status->setPausedStatus) {
    			$this->paused = 1;
    			if(property_exists ( $this, 'WorkElement' ) and $this->WorkElement->ongoing)$this->WorkElement->stop();
  ...
  public function buildSelectClause($included = false, $hidden = array(), $dep = array(), $parent = null) {
    ...
    foreach ( $this as $col => $val ) {
      ...
      if (! isset ( $dep [$col] ) and ! isset ( $dep ['root'] ) and (($included and ($col == 'id' or $threeCars == 'ref' or $threeCars == 'top' or $col == 'idle')) or ($firstCar == '_') or (pq_strpos ( $this->getFieldAttributes ( $col ), 'hidden' ) !== false and pq_strpos ( $this->getFieldAttributes ( $col ), 'forceExport' ) === false) or ($col == 'password') or (isset ( $hidden [$col] )) or (pq_strpos ( $this->getFieldAttributes ( $col ), 'noExport' ) !== false) or (pq_strpos ( $this->getFieldAttributes ( $col ), 'calculated' ) !== false))) 
  ...
  public function setReference($force = false, $old = null, $fmtPrefix=null, $fmtSuffix=null, $fmtNumber=null) {
    ...
    if (get_class ( $this ) == 'Document' and property_exists ( $this, 'documentReference' )) {
      ...
      $docRef = pq_str_replace ( array('{PROD}', '{PROJ/PROD}','{PROD/PROJ}','{PROJ}', '{TYPE}', '{NUM}', '{NAME}'), array(
          $prodObj->designation,
          ($projObj->projectCode)?$projObj->projectCode:$prodObj->designation,
      		($prodObj->designation)?$prodObj->designation:$projObj->projectCode,
          $projObj->projectCode, 
...
// MTY - LEAVE SYSTEM    
  ...
  public static function isWork($field) {
    if (pq_substr($field,-4)=='Work') return true;
    return false;
  }
  ...
  public static function isPct($field) {
    if (pq_substr($field, -3, 3)=='Pct' or pq_substr($field, -4, 4)=='Rate') return true;
    return false; 
  }
}
