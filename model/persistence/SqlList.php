...
//scriptLog("fetchList($listType,$displayCol, $selectedValue, $showIdle, $translate)");
    ...
    foreach ($crit as $col => $val) {
      if (get_class($obj)=='Language' and $col=='scope') continue; // Ticket #6377 - Errors found in logs.
    ...
    if (property_exists($obj,'_sortCriteriaForList') and pq_trim($obj->_sortCriteriaForList)) {
    ...
    } else if (property_exists($obj,'sbsSortable')) {
        $query .= ' order by ' . $obj->getDatabaseTableName() . '.sbsSortable, ' . $obj->getDatabaseTableName() . '.' . $obj->getDatabaseColumnName($displayCol); 
...
//         	$name=$nameObj->name;
      ...
      if ($displayCol=='name' and property_exists($obj,'_constructForName') and !$calculated ) {
        ...
        $nameObj=new $listType();
        $listNameObj=$nameObj->getSqlElementsFromCriteria(null,null,"id in ($in)",null, true);
        foreach ($res as $id=>$name) {
          $nameObj=$listNameObj["#$id"];
          if (method_exists($nameObj, 'setName')) $nameObj->setName();
          $res[$id]=$nameObj->name;
        }
      }
    }
  ...
  //scriptLog("fetchListWithCrit(listType=$listType,criteria=".implode('|',$criteria).",displayCol=$displayCol, selectedValue=$selectedValue, showIdle=$showIdle)");
    ...
    if (is_array($criteria)) {
      ...
      foreach ($crit as $col => $val) {
        if ( (pq_strtolower($listType)=='resource' or pq_strtolower($listType)=='contact' or pq_strtolower($listType)=='user' 
        or pq_strtolower($listType)=='accountable' or pq_strtolower($listType)=='affectable') and $col=='idProject') {
          $aff=new Affectation();
          $user=new Resource();
          if ($val=='*' or ! $val) {$val=0;}
        ...
        } else if ((pq_strtolower($listType)=='version'
            or pq_strtolower($listType)=='productversion' or pq_strtolower($listType)=='componentversion' 
            or pq_strtolower($listType)=='originalversion' or pq_strtolower($listType)=='originalproductversion' or pq_strtolower($listType)=='originalcomponentversion' 
            or pq_strtolower($listType)=='targetversion' or pq_strtolower($listType)=='targetproductversion' or pq_strtolower($listType)=='targetcomponentversion') and $col=='idProject') {
        	$vp=new VersionProject();
          $ver=new Version();
          $proj=new Project($val,true);
        ...
        } else if ( ( pq_strtolower($listType)=='componentversion'  
              or pq_strtolower($listType)=='originalcomponentversion'
              or pq_strtolower($listType)=='targetcomponentversion') and $col=='idProductVersion') {
        ...
        } else if (pq_strtolower($listType)=='indicator' and $col=='idIndicatorable' ) {
        ...
        } else if ( (pq_strtolower($listType)=='warningdelayunit' or pq_strtolower($listType)=='alertdelayunit') and $col=='idIndicator' ) {
          ...
          if ($val===null or $val=='' or $val=='null') {
            ...
            if ($col=='idProject' and ($val=='*' or ! pq_trim($val))) {continue;}
            $dataType=$obj->getDataType($col);
            if ($dataType=='numeric' or $dataType=='decimal' or $dataType=='int' or $dataType=='boolean') {
              if ($val!=' ') $query .= ' and ' . $obj->getDatabaseTableName() . '.' . $obj->getDatabaseColumnName($col) . '=' . Sql::fmtStr($val);
            } else { 
              $query .= ' and ' . $obj->getDatabaseTableName() . '.' . $obj->getDatabaseColumnName($col) . '=' . Sql::str($val);
            }
          }
    ...
    if (property_exists($obj,'_sortCriteriaForList') and $obj->_sortCriteriaForList) {
      ...
      $query .= ' order by ' . $obj->getDatabaseTableName() . '.sortOrder';
        ...
        $query .= ' order by ' . $obj->getDatabaseTableName() . '.sbsSortable';
    } else if (property_exists($obj,'order')) {
