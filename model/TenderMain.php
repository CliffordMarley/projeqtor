...
// ============================================================================**********
  ...
  public function save() {
    ...
    if(RequestHandler::getBoolean('generateProjectExpenseButton')){
      ...
      if($canCreate){
        if(pq_trim(RequestHandler::getValue('objectClassName'))==get_class($this)){
          ...
          $projExpense->name = pq_substr($this->name,0,100);
          ...
          // #3717 : end
          if(pq_trim($this->deliveryDate)){
            $projExpense->expensePlannedDate = $this->deliveryDate;
          }else if(pq_trim($this->receptionDateTime)){
            $projExpense->expensePlannedDate = pq_substr($this->receptionDateTime,0,10);
          }else if (pq_trim($this->requestDateTime)){
            $projExpense->expensePlannedDate = pq_substr($this->requestDateTime,0,10);
          }else{
    ...
    if ($this->idCallForTender and $this->idProvider) {
      ...
      $this->name=pq_substr($this->name, 0,200);
    }
    ...
    if (getLastOperationStatus($result)=='NO_CHANGE' and $resultEval!="" and getLastOperationStatus($resultEval)=="OK") {
      return pq_str_replace(array(getLastOperationMessage($result),'NO_CHANGE','#'.$resultEvalId),array(getLastOperationMessage($resultEval),"OK",'#'.$this->id),$result);
    }
  ...
  public function copyTo($newClass, $newType, $newName, $newProject, $setOrigin, $withNotes, $withAttachments, $withLinks, $withAssignments = false, $withAffectations = false, $toProject = NULL, $toActivity = NULL, $copyToWithResult = false, $copyToWithActivityPrice=false, $copyToWithStatus = false, $copyToWithSubTask=false) {
    ...
    return parent::copyTo($newClass, $newType, $newName, $newProject, $setOrigin, $withNotes, $withAttachments, $withLinks, $withAssignments, $withAffectations, $toProject, $toActivity, $copyToWithResult, $copyToWithActivityPrice, $copyToWithStatus);
  }
  // ============================================================================**********
  ...
  public static function drawListFromCriteria($crit,$val) {
    ...
    if (pq_strpos($param,'#'.pq_substr($crit,2).'#')==null) return;
