...
if ($mode=='REGUL' and $pe->realStartDate and !$pe->realEndDate and $pe->validatedEndDate>$pe->plannedEndDate) {
  $end=$pe->validatedEndDate;
} else if ($mode=='REGUL' and !$pe->realEndDate and $pe->plannedEndDate>$pe->validatedEndDate) {
...
$hideAssignationWithoutLeftWork=Parameter::getUserParameter('hideAssignationWihtoutLeftWork');
...
foreach($wkLst as $wk) {
  ...
  if ($hideAssignationWithoutLeftWork) {
    $ass=new Assignment($wk->idAssignment);
    if ($ass->leftWork == 0) {
      unset($work[$keyAss]);
    }
...
if ($pe->plannedEndDate>$end and !$pe->realEndDate) $end=$pe->plannedEndDate;
  ...
  if ($end<$wk->workDate) continue;
	...
	if(isset($work[$keyAss][$wk->workDate])){
	  if($work[$keyAss][$wk->workDate]['type']=='real'){
	    $work[$keyAss][$wk->workDate]=array('work'=>$work[$keyAss][$wk->workDate]['work'],
...
if (!$start or !$end) {
  if ($pe->elementary) {
    ...
    else if ($hideAssignationWithoutLeftWork) echo '<div style="background-color:#FFF0F0;padding:3px;border:1px solid #E0E0E0;">'.i18n('noDataToDisplay').'"</div>"';
...
foreach ($work as $resWork) {
  ...
  foreach ($dates as $dt) {
    ...
    if ($heightSurbooked>0 ){
      echo '<div style="display:block;background-color:#f4bf42;position:absolute;bottom:'.$height.'px;left:0px;width:100%;height:'.$heightSurbooked.'px;"></div>';
    }
  ...
  echo '</div><div style="position:absolute;left:10px;margin-top:-15px;text-shadow: 1px 1px 2px white;" class="planningBarDetailResName">'.$resWork['resource'].'&nbsp;</div></td>';
  echo '</tr>';
}
echo '</table>';
