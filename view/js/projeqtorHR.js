...
var wfStatusArray = [];
var idStatusSubmitter=0;
var popupTypeSelectOptions = [];
var arrayJustification=[];
...
function getWorkflowStatusesOfLeaveType(from,idLeaveType,idRes,idUser,nbDays,idStatus) {
    ...
    /*
     * Si la popup n'a pas était ouverte on set la variable just à 0 ce qui signifie que le champs commentaire ne sera pas obligatoire
     */
    if (dijit.byId('popupLeaveType') !== undefined) {
      var just=arrayJustification[dijit.byId('popupLeaveType').get('value')];
    } else {
      var just=1;
    }
    if(just==1 && dijit.byId('popupReason').get("required")!=true) {
      dijit.byId('popupReason').set("required", true);
      dijit.byId('popupReason').set("style", "border-left:2px solid red;");
    }else if(just!=1 && dijit.byId('popupReason').get("required")==true){
      dijit.byId('popupReason').set("required", false);
      dijit.byId('popupReason').set("style", "border-left:1px solid #d4d4d4;");
    }
    ...
    dojo.xhrGet({
        ...
        load : function(data) {
            if(data){
                ...
                idStatusSubmitter=res["idStatusSubmitter"];
                if (!(idLeaveType in statusFromTo)) {
                    statusFromTo[idLeaveType] = res["FromTo"][idLeaveType];
                }
...
function refreshStatusesSelect(theStatusArray, theSelectString, idLeaveType) {        
    ...
    if (theSelectString==='popupStatus') {
        ...
        if (!selectedStatusExist) {
            ...
            validateButtonCalendarPopup.set("style", "float:right");
            validateAndSubmittedButtonCalendarPopup.set("disabled", true);
            validateAndSubmittedButtonCalendarPopup.set("style", "display:none");
...
function fillLeavesCalendarDataArray(startDate,endDate){
    ...
    idEvent=0;
    dojo.xhrGet({
        url : "../tool/getLeaves.php?startDate="+startDate+"&endDate="+endDate+"&idRes="+idRes+"&csrfToken="+csrfToken,
        handleAs : "text",
        sync: true,
        load : function(data, args) {
...
function refreshEmployeeCustomLeavesEarned(){
    ...
    dojo.xhrGet({
        url : "../tool/getEmployeeCustomLeavesEarned.php?idEmployee="+idEmployee+"&csrfToken="+csrfToken,
        handleAs : "text",
        sync: true,
        load : function(data, args) {
...
function refreshEmployeeLeavesSummary(){
    ...
    //return lvType:(idLeaveType/color),lvTypeOfEmpContractType:(periodDuration),lvEarned:(startDate/endDate/quantity/leftQuantity), 
    dojo.xhrGet({
        url : "../tool/getEmployeeLeaveEarnedSummary.php?idEmployee="+idEmployee+"&csrfToken="+csrfToken,
        handleAs : "text",
        sync: true,
        load : function(data, args) {
            var lines = JSON.parse(data);
            ...
            var i118nDurationPeriod=i18n('colPeriodDuration');
            var summaryContent = '<b style="margin-bottom:5px">'+i18n("synthesisOfLeaveEarned")+'</b>';
            summaryContent += '<table style="width:96%; margin-left:2%; text-align:center; border: solid 1pt;">\n\
                                  <tr style="border: solid 1pt; height: 20px;">\n\
                                  <th style="text-align:center;"><b>'+ i18n('colIdLeaveType')+'</b></th>\n\
                                  <th style="text-align:center;"><b>'+ i118nDurationPeriod.replace(' - ','<br/>')+'</b></th>\n\
                                  <th style="text-align:center;"><b>'+ i18n('leavePeriod').toLowerCase()+'</b></th>\n\
                                  <th style="text-align:center;"><b>'+ i18n('colEarned')+'</b></th>\n\
                                  <th style="text-align:center;"><b>'+ i18n('recorded')+'</b></th>\n\
            ...
            for(var i=0;i<len;i++){
               ...
               summaryContent += '<td>'+lines[i].recorded+'</td>';
               summaryContent += '<td>'+lines[i].taken+'</td>';
               summaryContent += '<td>'+lines[i].left+'</td>';
               summaryContent += '<td>'+((lines[i].earnedPeriodPlusOne==0)?' - ':lines[i].earnedPeriodPlusOne)+'</td>';
               summaryContent += '</tr>';
...
function getFullStatusInfo(from,idStatus, idLeaveType) {
    ...
    if (wfStatusArray.length===0 || !(idLeaveType in statusFromTo) ) {
...
function showModifPopup(idLeave, leaveType, status, startDate, endDate, startAMPM, endAMPM, nbDays, reason, statusOK, statusUnknow){
    ...
    if(neutralStatus && theFullStatus!=null && statusOK!="1" && statusUnknow!="1"){
        ...
        validateButtonCalendarPopup.set("style", "");
        validateAndSubmittedButtonCalendarPopup.set("disabled", false);
        validateAndSubmittedButtonCalendarPopup.set("style", "display:inline-block");
        ...
        if (theFullStatus===null || statusOK=="1" || statusUnknow=="1") {
            ...
            validateButtonCalendarPopup.set("disabled", false);
            if(theFullStatus.setSubmittedLeave=="0"){
              validateButtonCalendarPopup.set("style", "");
            }else{
              validateButtonCalendarPopup.set("style", "float:right");
            }
        ...
        if(theFullStatus.setSubmittedLeave=="0" && theFullStatus.setRejectedLeave=="0" && theFullStatus.setAcceptedLeave=="0"){
        	validateAndSubmittedButtonCalendarPopup.set("disabled", false);
            validateAndSubmittedButtonCalendarPopup.set("style", "display:inline-block");
        }else{
        	validateAndSubmittedButtonCalendarPopup.set("disabled", true);
...
//to set the values of the fields of the popup before opening it
    ...
    //Put the first value in the select menu of leave type
    var theLeaveType=selectLeaveTypeFirst[0];
    if (leaveType) theLeaveType=leaveType;
    dijit.byId('popupLeaveType').attr('value', theLeaveType);
 ...
 * @returns {undefined}
 */    
function validPopupAction(status){
    if(dijit.byId('popupReason').get('required')==true && dijit.byId('popupReason').get('value').trim()==""){
      showAlert(i18n('messageMandatory',[i18n("reason")]));
      return;
    }
    showWait();
    disablePopupButtons();
    ...
    if(!status)status=dijit.byId("popupStatus").value;
...
function deletePopupAction(){
    ...
    if(creatBool === false){
       dojo.xhrGet({
            url : "../tool/deleteLeaveOfCalendar.php?idLeave="+dijit.byId("popupLeaveId").value+"&csrfToken="+csrfToken,
            handleAs : "text",
            sync: true,
            load : function(data, args) {
                var res=JSON.parse(data);
...
//                domConstruct.destroy(calendar.matrixView.yearColumnHeader);
            ...
            //Arrays to make the link between the names and ids of the leaveType
                ...
                for(var i = 0; i< lengthLvTArray; i++){
                    ...
                    if(leavesTypesArray[i]!=undefined){
                      arrayJustification[defaultLvTypeArray[i].id]=leavesTypesArray[i]._spe_isJustifiable;
                    }else{
                      arrayJustification[defaultLvTypeArray[i].id]=defaultLvTypeArray[i]._spe_isJustifiable;
                    }
            ...
            //a listener for the validate button of the popup, WIP
                ...
                validateAndSubmittedButtonCalendarPopup.on("click", function(e){
                  validPopupAction(idStatusSubmitter);                   
                  fillCalendarStore();
              });
...
function getLeftByLeaveType(idEmployee) {
    ...
    dojo.xhrGet({
        url : "../tool/getLeftQuantityByLeaveTypeForAnEmployee?idEmployee="+idEmployee+"&csrfToken="+csrfToken,
        handleAs : "text",
        sync: true,
        load : function(data, args) {
...
function refreshHierarchicalSkillList(){
  showWait();
  callback=function(){
    hideWait();
    if (dijit.byId('listDiv')) dijit.byId('listDiv').resize();
  }
  loadContent("../view/hierarchicalSkillView.php", "hierarchicalListDiv", null, false, null, null, null, callback, null);
}

function expandHierarchicalSkillGroup(idSkill, subSkill){
  expandHierarchicalGroup(idSkill, subSkill, 'Skill');
}


function moveSkillFromHierarchicalView (idFrom, idTo){
  var mode='before';
  dndSourceTableSkill.sync();
  var nodeList=dndSourceTableSkill.getAllNodes();
  ...
  var url='../tool/moveSkillFromHierarchicalView.php?idFrom=' + idFrom
  ...
  dojo.xhrPost({
    ...
    load : function() {
      ...
      refreshParentSkillList();
    }
  });
}

function refreshParentSkillList(){
  var selectedParent=dijit.byId('listSkillParentFilter').get('value');
  if (! selectedParent || selectedParent.trim()=='') selectedParent=null;
  var val=(dijit.byId('showClosed').get('value')=='on')?'all':0;
  refreshList('idSkill','idle',val,selectedParent,'listSkillParentFilter');
}

function changeDisplayDialogLvTypeOfEmpContractType(){
  var idLeaveType=dijit.byId("rightIdLeaveType").get("value");
  url='../tool/getSingleData.php?dataType=existingRightForLeaveType&id='+ idLeaveType;
  dojo.xhrGet({
    url : url+'&csrfToken='+csrfToken,
    handleAs : "text",
    load : function(data) {
      if(data){
        var display=(data=='true')?"none":"table-row";
        dojo.byId('leaveTypeNeedRight').value=(data=='true')?'false':'true';
        var allLignes= document.querySelectorAll("tr.ligneLeaveRight");
        allLignes.forEach(function(el){
